...
void FFT(vector<complex<ld>>& v, bool inverse) {
	int n = (int)v.size(); //Assumes n is a power of 2
	for (int i = 1, j = 0; i < n; i++) { //Sorting by reverse bits
		int bit = n >> 1;
		for (; j & bit; bit >>= 1) j ^= bit;
		if (i < (j ^= bit)) { swap(v[i], v[j]); }
	}
	...
	for (int len = 2; len <= n; len <<= 1) {
		...
		for (int i = 0; i < n; i += len) {
			...
			for (int j = 0; j < (len >> 1); j++, W *= Wn) {
				complex<ld> even = v[i + j], odd = W * v[i + j + (len >> 1)];
				v[i + j] = even + odd; v[i + j + (len >> 1)] = even - odd;
			}
		}
	}

	if (inverse) { for (auto& x : v) x /= n; }
}
...
int main() {
	...
	int k, n, m; cin >> k >> n >> m;
	freqA.resize(k + 1); freqB.resize(k + 1);
	for (int i = 0; i < n; i++) {
		int x; cin >> x; freqA[x]++;
	}
	for (int i = 0; i < m; i++) {
