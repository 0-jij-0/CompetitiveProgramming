...
string findKthSubstr(int l, int r, int dec, ll k, RMQ& rmq, vector<int> &SA, vector<ll> &pref, const string& s) {
	if (l == r) { return move(s.substr(SA[l], (int)k + dec)); }

	int i = rmq.query(l + 1, r), h = rmq.get(i) - dec, len = r - l + 1;
	ll leftCount = (ll)(h + dec) * (i - l);

	//LCP Part
	if ((ll)h * (ll)len < k) { k -= (ll)h * (ll)len; }
	else return move(s.substr(SA[l], dec + (int)((k + len - 1) / len)));

	//Left Part
	ll leftTotal = i ? pref[i - 1] - (l ? pref[l - 1] : 0) : 0;
	leftTotal = (ll)(i - l) * s.size() - leftTotal - leftCount;
	if (leftTotal < k) { k -= leftTotal; }
	else return move(findKthSubstr(l, i - 1, dec + h, k, rmq, SA, pref, s));

	//Right Part
	return move(findKthSubstr(i, r, dec + h, k, rmq, SA, pref, s));
}

string kthSubstr(string& s, ll k) {
	int n = (int)s.size(); vector<int> SA, LCP;
	suffixArray(s, SA, LCP); RMQ rmq(LCP);
	vector<ll> pref(n); pref[0] = SA[0];
	for (int i = 1; i < n; i++) pref[i] = pref[i - 1] + SA[i];
	return move(findKthSubstr(0, n - 1, 0, k, rmq, SA, pref, s));
}
