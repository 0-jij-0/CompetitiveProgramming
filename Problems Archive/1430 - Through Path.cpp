...
int main() {
	...
	int q; cin >> q; while (q--) {
		...
		int ae = edges[e].first, be = edges[e].second;
		if (t == 2) { swap(ae, be); }
		if (start[ae] < start[be]) {
			st.update(start[0], finish[0], x);
			st.update(start[be], finish[be], -x);
		}
		else {
			st.update(start[ae], finish[ae], x);
		}
	}

	for (int i = 0; i < n; i++)
		cout << st.query(start[i], start[i]).v << '\n';
