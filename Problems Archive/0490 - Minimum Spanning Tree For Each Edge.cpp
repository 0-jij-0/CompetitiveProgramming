...
struct graph {
	...
	graph KruskalMST(ll &res) {
		sort(edges.begin(), edges.end());
		DSU dsu(n); graph g(n); int i = 0; res = 0;
		while (i < m && g.edges.size() != n - 1) {
			edge &e = edges[i];
			if (!dsu.merge(e.u, e.v)) { i++; continue; }
			g.add_edge(e.u, e.v, e.w); res += e.w; i++;
		}
		return move(g);
	}
};
...
struct LCA {
	...
	int find(int u, int v) {
		...
		return max(res, max(parent[u].second, parent[v].second));
	}
};
...
int main() {
	...
	vector<edge> v = g.edges; ll res;
	g = g.KruskalMST(res); LCA lca(0, g);

	for (auto &e : v)
		cout << res + e.w - lca.find(e.u, e.v) << endl;
